...
import io.kinference.ndarray.utils.divUp
import io.kinference.ndarray.utils.shapeToWorkSize

class MatMulOperator : CachingShaderFunction() {
    override fun implementation(inputInfo: List<NDArrayInfo?>): NDArrayFunction =
        when {
            inputInfo[1]!!.shape.size >= 2 && inputInfo[1]!!.shape.takeLast(2).all { it % 4 == 0 } -> MatMulPackedVec4(inputInfo[0]!!, inputInfo[1]!!)
            else -> MatMulPackedUnaligned(inputInfo[0]!!, inputInfo[1]!!)
        }
}
...
) : MatMulPacked(inputInfo0, inputInfo1) {
    companion object {
